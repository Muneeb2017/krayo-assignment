...
function useMedia(mediaQueryString, defaultState) {
  ...
  const [matches, setMatches] = React__default.default.useState(() => {
    ...
    process.env.NODE_ENV !== "production" ? warning.warning(true, '`useMedia` When server side rendering, defaultState should be defined to prevent a hydration mismatches.') : void 0;
    return false;
  });
  React.useEffect(() => {
    if (features[mediaQueryString] !== undefined) {
